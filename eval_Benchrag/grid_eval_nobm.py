...
# 임베딩 모델 확인: 인덱스 생성 시 사용한 모델과 일치해야 함 (예: MongoDB/mdbr-leaf-ir)
...
async def main():
            ...
            for top_k in TOP_K_LIST:
                ...
                for ex in tqdm(data_samples, desc=desc):
                    ...
                    try:
                        ...
                        print(f"\n[ERROR] Retrieval failed for ID {q_id}: {e}")
                        continue
    ...
    csv_f.close()
    jsonl_f.close()
    print(f"\n[DONE] Vector RAG Experiment completed. Results: {OUTPUT_JSONL}")
